                ...
                {travelersData&&travelersData.map((passenger, index) => (
                    <>
                        ...
                        onClick={() => setDropdown((prev) => !prev)} 
                            key={index}
                            className="flex items-center justify-between mb-2 text-base h-28 bg-bluebg  p-4 rounded-lg border border-gray py-9"
                        >
                       ...
                       {dropdown&& <div className={`${dropdown ? 'h-auto pb-4' : 'h-0'} px-4 w-full lg:w-1/2 filledfields mx-auto transition-all duration-300`}>
                            <div className=" mt-5 rounded-xl p-7 bg-secondary shadow-lg border-gray border-[1px] h-fit">
                                ...
                                <p className=" py-4 flex justify-between border-b border-lightgray capitalize font-semibold text-white ">Mobile <span>{Object.values(passenger.mobile).map((values) => (values))}</span></p>
