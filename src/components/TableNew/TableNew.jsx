...
const TableNew = ({
  ...
  useEffect(() => {
    ...
    if ((activeIndex != null) & (extraRows != undefined)) {
      setExtraRow(
        Object.entries(tableData[activeIndex]).filter(([key]) =>
          extraRows.includes(key)
        )
      ); // Compare key with fieldName values
    }
  }, [activeIndex]);

  const columnsView = columnsToView
    .filter(
      (item) => item.fieldName !== "origin" && item.fieldName !== "destination"
    )
    .map((item, i) => {
      if (item.type == "number") {
        return {
                  ...
                  {row.original.role
                      ...
                      actions.map((action, idx) => (
                        <CustomTooltip key={idx} content={action.name}>
                          <div
                            ...
                            onClick={() =>
                              action.handler(row.index, tableData[row.index])
                            }
                          >
                    ...
                    : actions.map((action, idx) => (
                        <CustomTooltip key={idx} content={action.name}>
                          ...
                          >
                            <div className="ml-2">{action.icon}</div>
                          </div>
                        </CustomTooltip>
                      ))}
      ...
      <Modal
        ...
        Message={
          <Input
            label={"Role"}
            name={"Role"}
            value={editIdx != null && tableData[editIdx].role}
          />
        }
        toggle={true}
        btnText={"update"}
